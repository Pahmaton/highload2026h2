...
- При трёх ДЦ каждый держит около 53 % нагрузки (запас x1,5). Кворум "2 из 3" переживает потерю одного ДЦ, и каждая из _трёх_ реплик (п. 2.3.1) хранится в своём ДЦ.
...
```mermaid
flowchart LR
  App[Приложение] -->|1. api.bank.ru?| R[DNS-резолвер провайдера]
  R -->|2| NS["ns1/ns2.bank.ru (anycast 203.0.113.0/24)"]
  NS -->|3. адрес ДЦ по региону| R
  R -->|4| App
  App -->|"5а. HTTPS: ЦФО, ЮФО, СКФО, 50 % Москвы и МО"| DC1["ДЦ-1 Москва 198.51.100.0/24"]
  App -->|"5б. HTTPS: СЗФО, запад ПФО, 50 % Москвы и МО"| DC2["ДЦ-2 СПб 192.0.2.0/24"]
  App -->|"5в. HTTPS: восток ПФО, УФО, СФО, ДФО"| DC3["ДЦ-3 Екатеринбург 198.18.0.0/24"]
```
...
- В случае отказа DNS убирает ДЦ из ответов после health-check. Приложение при ошибке соединения или таймауте сразу повторяет запрос на резервный адрес (api-msk / api-spb / api-ekb по таблице отказов), не дожидаясь истечения TTL. Повтор платежа безопасен благодаря "ключу идемпотентности".
